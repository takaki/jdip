...
public class Utils {
    ...
    public static final Border EMPTY_BORDER_10 = new EmptyBorder(10, 10, 10,
            10);
    ...
    // private regex
    private static final Pattern REAL_COMMAS = Pattern
            .compile(",(?=([^\"]*\"[^\"]*\")*(?![^\"]*\"))");
    ...
    public static void centerIn(final Component inner, final Component outer) {
        ...
        if (componentSize.width > parentSize.width || componentSize.height > parentSize.height) {
            ...
            inner.setLocation(
                    pLoc.x + (parentSize.width - componentSize.width) / 2,
                    pLoc.y + (parentSize.height - componentSize.height) / 2);
        }
    }// centerIn()
    ...
    public static InputStream getInputStream(
        ...
        throw new FileNotFoundException(
                getLocalString(UTILS_RES_NOT_FOUND, name));
    }// getInputStream()
    ...
    private static void showNoLocalStringPopup(final String resourceKey,
        ...
        if (text == null) {
            ...
            text = MessageFormat
                    .format(text, new Object[]{resourceKey, e.getMessage()});
        }
                    ...
                    for (int i = 0; i < sRGB.length; i++) {
                        if (sRGB[i].endsWith("%") && sRGB[i].length() > 1) {
                            final int percent = Integer.parseInt(
                                    sRGB[i].substring(0, sRGB[i].length() - 1));
    ...
    public static JEditorPane createTextLabel(final String text, final boolean blend,
        ...
        if (!blend) {
            jep.setMargin(new Insets(TEXT_INSETS, TEXT_INSETS, TEXT_INSETS,
                    TEXT_INSETS));
        }
    ...
    public static String[] parseCSV(final String input) {
        ...
        final String[] matches = (String[]) matchList
     ...
     * returned.
     */
    public static String replaceAll(final String input, final String toFind,
