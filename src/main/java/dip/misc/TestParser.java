...
public class TestParser {
    ...
    private static final String KEYWORDS[][] = {{"variant", KEY_VARIANT}, {"setupdislodged", KEY_SETUPDISLODGED}, {"setup", KEY_SETUP}, {"end", KEY_END}, {"order", KEY_ORDER}, {"ord", KEY_ORDER}, {"result", KEY_RESULT}, {"res", KEY_RESULT}, {"#", KEY_COMMENT}};
    ...
    private static final String[][] ORDER_ARGS = {
            ...
            // simple types (3 args)
            {"hold", T_POWER, T_LOCATION, T_UTYPE}, {"disband", T_POWER, T_LOCATION, T_UTYPE}, {"build", T_POWER, T_LOCATION, T_UTYPE}, {"remove", T_POWER, T_LOCATION, T_UTYPE},
            // complex types
            {"move", T_POWER, T_LOCATION, T_UTYPE, T_LOCATION, T_BOOLEAN}, {"retreat", T_POWER, T_LOCATION, T_UTYPE, T_LOCATION}, {"convoy", T_POWER, T_LOCATION, T_UTYPE, T_LOCATION, T_UTYPE, T_LOCATION}, {"support", T_POWER, T_LOCATION, T_UTYPE, T_LOCATION, T_UTYPE, T_LOCATION},};

    // 3 main phase types for proper validation
    private static final Phase PHASE_MOVE = new Phase(SeasonType.FALL, 1900,
            PhaseType.MOVEMENT);
    private static final Phase PHASE_ADJUSTMENT = new Phase(SeasonType.FALL,
            1900, PhaseType.ADJUSTMENT);
    private static final Phase PHASE_RETREAT = new Phase(SeasonType.FALL, 1900,
            PhaseType.RETREAT);
    ...
    private boolean valBasicParams(ORPair orp, Order o, String[] toks,
        ...
        if (isOK) {
            isOK = valUnitType(orp, o.getSourceUnitType(), toks[3],
                    failedCases);
        }
        ...
        if (tok.equalsIgnoreCase("true")) {
            ...
            System.out.println(
                    "ERROR: in result of order pair starting at line: " + orp
                            .getLineNumber());
            System.out.println(
                    "Boolean value \"" + tok + "\" must be \"true\" or \"false\".");
            System.exit(1);
        }
    ...
    private void setupVariant() {
        ...
        if (System.getProperty("user.dir") == null) {
            ...
            defaultVariantSearchDir = new File(System.getProperty("user.dir"),
                    VARIANT_DIR);
        }

        try {
            // parse variants
            VariantManager.init(new File[]{defaultVariantSearchDir});
        ...
        System.out.println(
                "Variant \"" + variantName + "\" loaded successfully.");
            ...
            while (line != null) {
                ...
                if (line.length() > 0) {
                    ...
                    if (key == KEY_VARIANT) {
                        ...
                        if (accum == null) {
                            lnrErrorExit(lnr,
                                    "END line must be after a SETUP line or SETUPDISLODGED line.");
                        }
                        ...
                        if (posList == null) {
                            lnrErrorExit(lnr,
                                    "SETUPDISLODGED must be after a SETUP block.");
                        }
                        ...
                        if (!setupDone) {
                            lnrErrorExit(lnr,
                                    "ORDER (or ORD) keyword must be after SETUP block complete.");
                        }

                        if (currentCase != null) {
                            lnrErrorExit(lnr,
                                    "ORDER (or ORD) keyword must precede and be paired with a RESULT (or RES) line.");
                        }
                        ...
                        if (!setupDone) {
                            lnrErrorExit(lnr,
                                    "RESULT (or RES) keyword must be after SETUP block complete.");
                        }

                        if (currentCase == null) {
                            lnrErrorExit(lnr,
                                    "RESULT (or RES) line must follow and be paired with an ORDER (or ORD) line.");
                        }
                        ...
                        if (accum != null) {
                            ...
                            System.out.println(
                                    "Unknown action or non-comment line.");
                            System.exit(1);
                        }
                    }
                }
