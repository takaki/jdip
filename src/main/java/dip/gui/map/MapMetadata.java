...
public class MapMetadata {
     ...
     */
    private static final Point2D.Float POINT_ZERO = new Point2D.Float(0.0f,
            0.0f);
	...
	*/
    ...
    private Object getOrderParam(String el, String att) {
        ...
        if (value == null) {
            throw new IllegalArgumentException(
                    "order parameters: key/attribute: " + el + "," + att + " not found!");
        }
    ...
    private void parsePlacements(Element root) throws MapException {
        ...
        NodeList nl = root
                .getElementsByTagNameNS(JDIP_NAMESPACE, EL_PROVINCE_DATA);
        if (nl.getLength() != 1) {
        ...
        for (int i = 0; i < nl.getLength(); i++) {
            try {
                ...
                if (province == null) {
                    throw new MapException(
                            "SVG error in PROVINCE tag: Province name=\"" + provinceName + "\" not recognized.");
                }
    ...
    private void parseDisplayMetadata(Element root) throws MapException {
        ...
        if (el != null) {
            displayProps
                    .put(ATT_ZOOM_MIN, el.getAttribute(ATT_ZOOM_MIN).trim());
            displayProps
                    .put(ATT_ZOOM_MAX, el.getAttribute(ATT_ZOOM_MAX).trim());
            displayProps.put(ATT_ZOOM_FACTOR,
                    el.getAttribute(ATT_ZOOM_FACTOR).trim());
        }

        el = getElement(displayRoot, EL_LABELS);
        if (el != null) {
            displayProps.put(ATT_LABELS_BRIEF,
                    el.getAttribute(ATT_LABELS_BRIEF).trim());
            displayProps.put(ATT_LABELS_FULL,
                    el.getAttribute(ATT_LABELS_FULL).trim());
        }
    ...
    private void parseOrderDrawingData(Element root) throws MapException {
        ...
        putOrderParam(EL_HOLD, ATT_DELTA_RADIUS,
                parseFloat(EL_HOLD, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));
        putOrderParam(EL_HOLD, ATT_STROKESTYLE,
                el.getAttribute(ATT_STROKESTYLE).trim());
        putOrderParam(EL_HOLD, ATT_HILIGHT_OFFSET,
                parseFloat(EL_HOLD, ATT_HILIGHT_OFFSET,
                        el.getAttribute(ATT_HILIGHT_OFFSET)));
        putOrderParam(EL_HOLD, ATT_HILIGHT_CLASS,
                el.getAttribute(ATT_HILIGHT_CLASS).trim());
        putOrderParam(EL_HOLD, ATT_WIDTHS, parseFloatArray(EL_HOLD, ATT_WIDTHS,
                el.getAttribute(ATT_WIDTHS)));
        putOrderParam(EL_HOLD, ATT_SHADOW_WIDTHS,
                parseFloatArray(EL_HOLD, ATT_SHADOW_WIDTHS,
                        el.getAttribute(ATT_SHADOW_WIDTHS)));
        putOptionalOrderParam(EL_HOLD, ATT_FILTERID,
                el.getAttribute(ATT_FILTERID).trim());

        // DISBAND
        el = getElement(orderRoot, EL_DISBAND);
        checkElement(EL_DISBAND, el);
        putOrderParam(EL_DISBAND, ATT_DELTA_RADIUS,
                parseFloat(EL_DISBAND, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));

        // REMOVE
        el = getElement(orderRoot, EL_REMOVE);
        checkElement(EL_REMOVE, el);
        putOrderParam(EL_REMOVE, ATT_DELTA_RADIUS,
                parseFloat(EL_REMOVE, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));

        // BUILD
        el = getElement(orderRoot, EL_BUILD);
        checkElement(EL_BUILD, el);
        putOrderParam(EL_BUILD, ATT_DELTA_RADIUS,
                parseFloat(EL_BUILD, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));

        // WAIVE
        el = getElement(orderRoot, EL_WAIVE);
        checkElement(EL_WAIVE, el);
        putOrderParam(EL_WAIVE, ATT_DELTA_RADIUS,
                parseFloat(EL_WAIVE, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));

        // MOVE
        el = getElement(orderRoot, EL_MOVE);
        checkElement(EL_MOVE, el);
        putOrderParam(EL_MOVE, ATT_DELTA_RADIUS,
                parseFloat(EL_MOVE, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));
        putOrderParam(EL_MOVE, ATT_STROKESTYLE,
                el.getAttribute(ATT_STROKESTYLE).trim());
        putOrderParam(EL_MOVE, ATT_MARKERID,
                el.getAttribute(ATT_MARKERID).trim());
        putOrderParam(EL_MOVE, ATT_HILIGHT_OFFSET,
                parseFloat(EL_MOVE, ATT_HILIGHT_OFFSET,
                        el.getAttribute(ATT_HILIGHT_OFFSET)));
        putOrderParam(EL_MOVE, ATT_HILIGHT_CLASS,
                el.getAttribute(ATT_HILIGHT_CLASS).trim());
        putOrderParam(EL_MOVE, ATT_WIDTHS, parseFloatArray(EL_MOVE, ATT_WIDTHS,
                el.getAttribute(ATT_WIDTHS)));
        putOrderParam(EL_MOVE, ATT_SHADOW_WIDTHS,
                parseFloatArray(EL_MOVE, ATT_SHADOW_WIDTHS,
                        el.getAttribute(ATT_SHADOW_WIDTHS)));
        putOptionalOrderParam(EL_MOVE, ATT_FILTERID,
                el.getAttribute(ATT_FILTERID).trim());

        // RETREAT
        el = getElement(orderRoot, EL_RETREAT);
        checkElement(EL_RETREAT, el);
        putOrderParam(EL_RETREAT, ATT_DELTA_RADIUS,
                parseFloat(EL_RETREAT, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));
        putOrderParam(EL_RETREAT, ATT_STROKESTYLE,
                el.getAttribute(ATT_STROKESTYLE).trim());
        putOrderParam(EL_RETREAT, ATT_MARKERID,
                el.getAttribute(ATT_MARKERID).trim());
        putOrderParam(EL_RETREAT, ATT_HILIGHT_OFFSET,
                parseFloat(EL_RETREAT, ATT_HILIGHT_OFFSET,
                        el.getAttribute(ATT_HILIGHT_OFFSET)));
        putOrderParam(EL_RETREAT, ATT_HILIGHT_CLASS,
                el.getAttribute(ATT_HILIGHT_CLASS).trim());
        putOptionalOrderParam(EL_RETREAT, ATT_FILTERID,
                el.getAttribute(ATT_FILTERID).trim());

        // SUPPORT
        el = getElement(orderRoot, EL_SUPPORT);
        checkElement(EL_SUPPORT, el);
        putOrderParam(EL_SUPPORT, ATT_DELTA_RADIUS,
                parseFloat(EL_SUPPORT, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));
        putOrderParam(EL_SUPPORT, ATT_STROKESTYLE,
                el.getAttribute(ATT_STROKESTYLE).trim());
        putOrderParam(EL_SUPPORT, ATT_MARKERID,
                el.getAttribute(ATT_MARKERID).trim());
        putOrderParam(EL_SUPPORT, ATT_HILIGHT_OFFSET,
                parseFloat(EL_SUPPORT, ATT_HILIGHT_OFFSET,
                        el.getAttribute(ATT_HILIGHT_OFFSET)));
        putOrderParam(EL_SUPPORT, ATT_HILIGHT_CLASS,
                el.getAttribute(ATT_HILIGHT_CLASS).trim());
        putOptionalOrderParam(EL_SUPPORT, ATT_FILTERID,
                el.getAttribute(ATT_FILTERID).trim());

        // CONVOY
        el = getElement(orderRoot, EL_CONVOY);
        checkElement(EL_CONVOY, el);
        putOrderParam(EL_CONVOY, ATT_DELTA_RADIUS,
                parseFloat(EL_CONVOY, ATT_DELTA_RADIUS,
                        el.getAttribute(ATT_DELTA_RADIUS)));
        putOrderParam(EL_CONVOY, ATT_STROKESTYLE,
                el.getAttribute(ATT_STROKESTYLE).trim());
        putOrderParam(EL_CONVOY, ATT_MARKERID,
                el.getAttribute(ATT_MARKERID).trim());
        putOrderParam(EL_CONVOY, ATT_HILIGHT_OFFSET,
                parseFloat(EL_CONVOY, ATT_HILIGHT_OFFSET,
                        el.getAttribute(ATT_HILIGHT_OFFSET)));
        putOrderParam(EL_CONVOY, ATT_HILIGHT_CLASS,
                el.getAttribute(ATT_HILIGHT_CLASS).trim());
        putOptionalOrderParam(EL_CONVOY, ATT_FILTERID,
                el.getAttribute(ATT_FILTERID).trim());
    ...
    private Point2D.Float parseCoord(String el, String att,
        ...
        throw new MapException(
                el + " attribute " + att + " value \"" + in + "\" is not specified or not a valid floating point value pair. (e.g., \"3.0, 2.01\")");
    }// parseCoord()
    ...
    private float[] parseFloatArray(String el, String att,
        ...
        if (arr.length == 0) {
            throw new MapException(
                    el + " attribute " + att + " value \"" + value + "\" must contain at least one positive floating point value.");
        }
