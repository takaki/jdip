...
public class OrderControlBar extends ViewControlBar {
    ...
    // il8n
    private final static String[] GROUP_MOVEMENT_TEXT = Utils
            .getLocalStringArray("OrdConBar.icons.movement");
    ...
    private final static int[] GROUP_MOVEMENT_CHARCODES = Utils
            .getLocalIntArray("OrdConBar.keys.movement");
    private final static String GROUP_MOVEMENT_DEFAULT_BUTTON = MODE_MOVE;

    private final static String[] GROUP_RETREAT_TEXT = Utils
            .getLocalStringArray("OrdConBar.icons.retreat");
    ...
    private final static int[] GROUP_RETREAT_CHARCODES = Utils
            .getLocalIntArray("OrdConBar.keys.retreat");
    private final static String GROUP_RETREAT_DEFAULT_BUTTON = MODE_RETREAT;

    private final static String[] GROUP_ADJUSTMENT_TEXT = Utils
            .getLocalStringArray("OrdConBar.icons.adjustment");
    ...
    private final static int[] GROUP_ADJUSTMENT_CHARCODES = Utils
            .getLocalIntArray("OrdConBar.keys.adjustment");
    ...
    public OrderControlBar(final MapPanel mp) {
        ...
        stateInfo.setValidationOptions(
                mapPanel.getClientFrame().getValidationOptions());
    ...
    public void mouseOver(final MouseEvent me, final Location loc) {
        ...
        if (currentOrder != null) {
            ...
            if (currentOrder.testLocation(stateInfo, loc, sb)) {
                ...
                if (inDrag && !dragLoc.equals(loc) && currentOrder
                        .getSourceUnitType() != null) {
                    if (MODE_MOVE == currentAction) {
                        tempOrder = (GUIMove) guiOrderFactory
                                .createMove(currentOrder.getPower(), dragLoc,
                                        currentOrder.getSourceUnitType(), loc,
                                        ((GUIMove) currentOrder).isViaConvoy());
                        ...
                        mapPanel.getOrderDisplayPanel()
                                .addOrder(tempOrder, false);
                    } else if (MODE_RETREAT == currentAction) {
                        tempOrder = (GUIRetreat) guiOrderFactory
                                .createRetreat(currentOrder.getPower(), dragLoc,
                                        currentOrder.getSourceUnitType(), loc);
                        mapPanel.getOrderDisplayPanel()
                                .addOrder(tempOrder, false);
    ...
    public void mouseOut(final MouseEvent me, final Location loc) {
        ...
        if (currentOrder == null) {
            ...
            mapPanel.setMapCursor(MapPanel.BAD_ACTION);
            mapPanel.getStatusBarUtils()
                    .setText(Utils.getLocalString("OrdConBar.noselection"));
        } else {
            handleNullLoc();
        }
    }
    ...
    public void doOrder(final MouseEvent me, Location loc) {
        if (currentOrder != null) {
            ...
            if (currentOrder.setLocation(stateInfo, loc, sb)) {
                ...
                mapPanel.getClientFrame().getOrderStatusPanel().setOrderText(
                        currentOrder.toFormattedString(
                                mapPanel.getClientFrame().getOFO()));
                mapPanel.getStatusBarUtils()
                        .displayProvinceInfo(loc, sb.toString());
                ...
                mapPanel.setMapCursor(MapPanel.BAD_ACTION);
                mapPanel.getStatusBarUtils().displayProvinceInfo(loc,
                        Utils.getLocalString(GUIOrder.CANCELED,
            ...
            if (currentOrder.isComplete()) {
                ...
                mapPanel.getClientFrame().getOrderStatusPanel()
                        .clearOrderText();
                mapPanel.setMapCursor(MapPanel.BAD_ACTION);
                setGUIOrder(loc);
                // create a new order writer
    ...
    private void handleNullLoc() {
        ...
        sb.setLength(0);
        sb.append(Utils.getLocalString("OrdConBar.noprovince.nostart",
                currentOrder.getFullName()));
    ...
    private void doCanceled() {
        if (currentOrder != null) {
            mapPanel.setMapCursor(MapPanel.BAD_ACTION);
            mapPanel.getStatusBarUtils().setText(
                    Utils.getLocalString(GUIOrder.CANCELED,
                            currentOrder.getFullName()));
            currentOrder = null;
            setGUIOrder(lastLoc);
