...
public class SVGUtils {
     ...
     */
    public static void setTitle(final SVGDocument document, final SVGElement element,
                                final String title) {
        Node titleNode = null;
        Node textNode = null;
     ...
     */
    public static void setDescription(final SVGDocument document, final SVGElement element,
     ...
     * <p>
     */
    public static SVGUseElement createUseElement(final SVGDocument document,
        ...
        useElement.setAttributeNS(null, SVGConstants.SVG_X_ATTRIBUTE,
                floatToString(x));
        useElement.setAttributeNS(null, SVGConstants.SVG_Y_ATTRIBUTE,
                floatToString(y));
        useElement.setAttributeNS(XLinkSupport.XLINK_NAMESPACE_URI,
                SVGConstants.SVG_HREF_ATTRIBUTE, symbolName);

        if (symbolSize != null) {
            useElement.setAttributeNS(null, SVGConstants.SVG_WIDTH_ATTRIBUTE,
                    symbolSize.getWidth());
            useElement.setAttributeNS(null, SVGConstants.SVG_HEIGHT_ATTRIBUTE,
                    symbolSize.getHeight());
        }
        ...
        if (attClass != null) {
            useElement.setAttributeNS(null, SVGConstants.SVG_CLASS_ATTRIBUTE,
                    attClass);
        }
	...
	*/
    ...
    private static void nodeWalker(final Node node, final List<String> list, final Map map,
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE && ((anySVGElement && node instanceof org.w3c.dom.svg.SVGElement) || (node
            ...
            if (node.hasAttributes()) {
                ...
                final Node attrNode = attributes.getNamedItem(
                        SVGConstants.SVG_ID_ATTRIBUTE);    // was ATTR_ID
                if (attrNode != null) {
     ...
     */
    private static void idNodeWalker(final Node node, final List<SVGElement> list, final boolean isRoot) {
        if (node.getNodeType() == Node.ELEMENT_NODE && !isRoot) {
            final String id = ((Element) node)
     ...
     */
    private static void nodeChecker(final Node attributeNode, final Node parentNode,
                                    final List<String> list, final Map<Object, Node> map) {
        ...
        while (iter.hasNext()) {
            ...
            if (obj == null) {
                ...
                if (obj instanceof Province) {
                    ...
                    final String[] provShortNames = ((Province) obj).getShortNames().toArray(new String[0]);
                    for (String provShortName : provShortNames) {
     ...
     * Case Insensitive.
     */
    public static Node findNodeWithID(final Node node, final String id) {
        ...
        if (children != null) {
            for (int i = 0; i < children.getLength(); i++) {
                final Node aNode = findNodeWithID(children.item(i), id);
                if (aNode != null) {
                    return aNode;
                }
            }
     ...
     */
    private static StringBuffer floatToSB(final float v) {
