...
public class GUIMoveExplicit extends Move implements GUIOrder {
     ...
     * Creates a GUIMoveExplicit
     */
    protected GUIMoveExplicit(Power power, Location source,
     ...
     */
    private LocationTestResult testLocationLTR(StateInfo stateInfo,
                                               Location location,
                                               StringBuffer sb) {
        sb.setLength(0);
            ...
            if (lastProv.isAdjacent(coast, province)) {
                ...
                if (province.isCoastal()) {
                    ...
                    sb.append(Utils.getLocalString(
                            FINAL_DESTINATION));    // final destination message
                    return result;
                } else {
                    if (province.isConvoyable() && position
                        ...
                        sb.append(Utils.getLocalString(
                                BAD_CONVOY_LOCATION));    // invalid location for convoy path
                        result.isValid = false;
                        return result;
                    }
                }
                ...
                sb.append(Utils.getLocalString(
                        NONADJACENT_CONVOY_LOCATION));    // non-adjacent location
                result.isValid = false;
                return result;
            }
        } else {
     ...
     * in that case.
     */
    private boolean testNonConvoyDest(StateInfo stateInfo, Location location,
                                      StringBuffer sb) {
        ...
        if (province.equals(src.getProvince())) {
            ...
            sb.append(Utils.getLocalString(CLICK_TO_SET_DEST));
            return true;
        } else if (!GUIOrderUtils
                .checkBorder(this, location, srcUnitType, stateInfo.getPhase(),
                        sb)) {
        ...
        if (ltr.isValid) {
            if (currentLocNum == 0) {
                ...
                if (ltr.isFinalDest) {
                    ...
                    sb.setLength(0);
                    sb.append(Utils.getLocalString(ADDED_CONVOY_LOCATION,
    ...
    private void updateConvoyPath() {
        if (tmpConvoyPath == null) {
            ...
            final Province[] provinceRoute = (Province[]) tmpConvoyPath
                    .toArray(new Province[tmpConvoyPath.size()]);
    ...
    private SVGElement drawNCOrder(MapInfo mapInfo, float offset,
        ...
        if (position.hasUnit(dest.getProvince())) {
            Unit.Type destUnitType = position.getUnit(dest.getProvince()).orElse(null)
                    .getType();
            float r = mmd.getOrderRadius(MapMetadata.EL_MOVE,
        ...
        line.setAttributeNS(null, SVGConstants.SVG_CLASS_ATTRIBUTE,
                mmd.getOrderParamString(MapMetadata.EL_MOVE,
