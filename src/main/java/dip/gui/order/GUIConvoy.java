...
public class GUIConvoy extends Convoy implements GUIOrder {
    ...
    public boolean testLocation(StateInfo stateInfo, Location location,
        ...
        if (currentLocNum == 0) {
                ...
                if (unit.getType() == Unit.Type.FLEET) {
                    if (province.isSea() || province.isConvoyableCoast()) {
                        ...
                        sb.append(Utils.getLocalString(
                                ONLY_SEA_OR_CC_FLEETS_CAN_CONVOY));
                        return false;
                    }
                } else {
                    sb.append(Utils.getLocalString(
            ...
            if (unit != null) {
                if (unit.getType() == Unit.Type.ARMY) {
                    if (province.isCoastal()) {
                        ...
                        sb.append(
                                Utils.getLocalString(CANNOT_CONVOY_LANDLOCKED));
                        return false;
                    }
                } else {
            ...
            if (stateInfo.getValidationOptions()
                ...
                sb.append(Utils.getLocalString(CLICK_TO_CONVOY_FROM,
            ...
            if (province.isCoastal()) {
                ...
                if (path.isPossibleConvoyRoute(convoySrc,
                    ...
                    // check borders
                    if (!GUIOrderUtils
                            .checkBorder(this, location, convoyUnitType,
                    ...
                    sb.append(Utils.getLocalString(NO_POSSIBLE_CONVOY_PATH,
                            convoySrc.getProvince().getFullName()));
                    return false;
                }
            } else {
    ...
    public boolean setLocation(StateInfo stateInfo, Location location,
        ...
        if (testLocation(stateInfo, location, sb)) {
            if (currentLocNum == 0) {
                ...
                convoyDest = new Location(location.getProvince(),
                        location.getCoast());
    ...
    private SVGElement[] drawOrder(MapInfo mapInfo, float offset,
        ...
        Point2D.Float newPtTo = GUIOrderUtils
                .getLineCircleIntersection(ptSrc.x, ptSrc.y, ptConvoySrc.x,
                        ptConvoySrc.y, ptConvoySrc.x, ptConvoySrc.y, radius);
        ...
        elements[0].setAttributeNS(null, SVGConstants.SVG_CLASS_ATTRIBUTE,
                mmd.getOrderParamString(MapMetadata.EL_CONVOY,
        ...
        if (position.hasUnit(convoyDest.getProvince())) {
            ...
            newPtTo = GUIOrderUtils
                    .getLineCircleIntersection(newPtFrom.x, newPtFrom.y,
                            ptConvoyDest.x, ptConvoyDest.y, ptConvoyDest.x,
                            ptConvoyDest.y, moveRadius);
        } else {
            newPtTo = ptConvoyDest;
        }

        elements[2] = (SVGLineElement) mapInfo.getDocument()
                .createElementNS(SVGDOMImplementation.SVG_NAMESPACE_URI,
                        SVGConstants.SVG_LINE_TAG);

        elements[2].setAttributeNS(null, SVGConstants.SVG_X1_ATTRIBUTE,
                GUIOrderUtils.floatToString(newPtFrom.x));
        elements[2].setAttributeNS(null, SVGConstants.SVG_Y1_ATTRIBUTE,
                GUIOrderUtils.floatToString(newPtFrom.y));
        elements[2].setAttributeNS(null, SVGConstants.SVG_X2_ATTRIBUTE,
                GUIOrderUtils.floatToString(newPtTo.x));
        elements[2].setAttributeNS(null, SVGConstants.SVG_Y2_ATTRIBUTE,
                GUIOrderUtils.floatToString(newPtTo.y));
